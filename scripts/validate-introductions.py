...
for index, item in enumerate(items):
    ...
    ids.add(item["id"])
    assert item["dates"] and all(date.startswith("2026-08-") for date in item["dates"]), f"invalid dates: {item['id']}"
...
print(f"INTRODUCTION DATA VALIDATION PASSED: {len(items)} cards with {len(source_refs)} numbered official HTTPS sources.")
